...
class PCViewsLoc:
    ...
    def get_img(self, points,RESOLUTION_x=224, RESOLUTION_y=224):
        """Get image based on the prespecified specifications.
        ...
        img,loc = points2depthloc(
            points=_points,
            image_height=RESOLUTION_x,
            image_width=RESOLUTION_y,
            size_x=1,
            size_y=1,
        )
        return img,loc
...
def distribute_loc(depth, _x, _y, size_x, size_y, image_height, image_width):
    """
    Distributes the depth associated with each point to the discrete coordinates (image_height, image_width) in a region
    of size (size_x, size_y).
    :param depth:
    ...
    # return weighed_value_scattered,  weight_scattered
    return weighed_value_scattered,  weight_scattered, torch.stack([extended_x[:,:,0,0],extended_y[:,:,0,0]],dim=-1).long()
    return weighed_value_scattered,  weight_scattered, torch.stack([extended_x[:,:,0,0],extended_y[:,:,0,0]],dim=-1).long()

def points2depthloc(points, image_height, image_width, size_x=4, size_y=4):
    """
    :param points: [B, num_points, 3]
    :param image_width:
    :param image_height:
    ...
    #     image_height=image_height,
    #     image_width=image_width)
    weighed_value_scattered, weight_scattered, loc = distribute_loc(
        depth=depth,
        _x=_x,
        _y=_y,
        size_x=size_x,

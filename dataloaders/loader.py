...
def sample_pointcloud(data_path, num_point, pc_attribs, pc_augm, pc_augm_config, scan_name,
    ...
    if random_sample:
        ...
        valid_point_inds = np.nonzero(data[:,-1] == sampled_class)[0]  # indices of points belonging to the sampled class
    ...
    rgb = data[:, 3:-1]    # modified for scenenn, as includes many attributes
    labels = data[:, -1].astype(np.int)    # modified for scenenn, as includes many attributes
    ...
    if 'rgb' in pc_attribs:
        if rgb.shape[-1]==3:
            ...
            ptcloud.append(rgb)     # modified for scenenn, as includes many attributes
    if 'XYZ' in pc_attribs: ptcloud.append(XYZ)
...
def augment_pointcloud(P, pc_augm_config):
    ...
    # if pc_augm_config['dropout'] > 1:
    #     dropout_ratio = np.random.rand()*0.1
    #     n = P.shape[0]
    #     idx = np.random.choice(n, int(n * dropout_ratio), replace=False)
    #     P[idx]=0
    return P
...
class MyDataset(Dataset):
    def __init__(self, data_path, dataset_name, cvfold=0, num_episode=50000, n_way=3, k_shot=5, n_queries=1,
        ...
        if dataset_name == 's3dis':
            ...
            self.dataset = SceneNNDataset(cvfold, data_path)
        elif dataset_name == 'nyudepthv2':
            from dataloaders.nyudepthv2 import NYUDepthV2Dataset
            self.dataset = NYUDepthV2Dataset(cvfold, data_path)
        elif dataset_name == 'semantic3d':
            from dataloaders.semantic3d import Semantic3DDataset
            self.dataset = Semantic3DDataset(cvfold, data_path)
        else:

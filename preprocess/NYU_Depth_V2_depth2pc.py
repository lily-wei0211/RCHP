...
https://github.com/parkie0517/NYUDepthV2_PointCloud_Converter/tree/main
https://github.com/parkie0517/NYUDepthV2_PointCloud_Converter/blob/main/depth_2_pc_complete.py
'''
import numpy as np
...
def convert_rgbd2pc(rgb, depth, label, scale):
    """1. Settings before we begin"""
    # create paths for the input images
    # depth_path = os.path.join(input_path, "
    # {file_base}.png")

    # read the images
    # depth_image = imageio.imread(depth_path)  # (480, 640, 3)
    # label_image = imageio.imread(label_path)
    # rgb_image = imageio.imread(rgb_path)  # (480, 640, 3)
    depth_image = depth
    label_image = label
    rgb_image = rgb
    ...
    """ 6. Save Data """
    # sample = {'coord': scaled_depth_PC, 'color': reshaped_rgb, 'semantic_gt': label_GT, 'normal': reshpaed_normal}
    # data_label = np.concatenate([scaled_depth_PC, reshaped_rgb, label_GT], -1)
    sample = {'coord': scaled_depth_PC, 'color': reshaped_rgb, 'semantic_gt': label_GT, 'normal': reshpaed_normal}
    return sample
...
if __name__ == "__main__":
    # define some things
    input_path = "/root/datasets/NYUDepthV2"  # change this!
    output_path = "/root/datasets/NYUDepthV2/dataset"  # change this!
    num_data = 1449
    # scale = 1000.0  # divides the point cloud data
    scale = 1.0  # divides the point cloud data
    # scale = 0.5  # divides the point cloud data

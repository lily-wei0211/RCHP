...
from models.rchp_learner import RCHPLearner
    ...
    for i, batch_gt_labels in enumerate(gt_labels_list):
        ...
        for j in range(batch_pred_labels.shape[0]):
            for k in range(batch_pred_labels.shape[1]):
                ...
                # print('batch_pred_labels: ',batch_pred_labels.shape, 'batch_pred_labels[j, k]: ',batch_pred_labels[j, k].shape)
...
def test_few_shot(test_loader, learner, logger, test_classes):
    ...
    for batch_idx, (data, sampled_classes,index) in enumerate(test_loader):
        ...
        if (batch_idx + 1) % 100 == 0:
            logger.cprint('[Eval] Iter: %d | Loss: %.4f | ACC: %.4f | %s' % (batch_idx + 1, loss.detach().item(), accuracy, str(datetime.now())))
...
def eval(args):
    ...
    if args.phase == 'rchpeval':
        learner = RCHPLearner(args, mode='test')

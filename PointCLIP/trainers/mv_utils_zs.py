...
#     _i = torch.linspace(-size_x / 2, (size_x / 2) - 1, size_x, requires_grad=False, device=depth.device)
#     _j = torch.linspace(-size_y / 2, (size_y / 2) - 1, size_y, requires_grad=False, device=depth.device)
#     # _i = torch.linspace(0,size_x, size_x, requires_grad=False, device=depth.device)
#     # _j = torch.linspace(0,size_x, size_y, requires_grad=False, device=depth.device)
#
#     extended_x = _x.unsqueeze(2).repeat([1, 1, size_x]) + _i  # [batch, num_points, size_x]
#     extended_y = _y.unsqueeze(2).repeat([1, 1, size_y]) + _j  # [batch, num_points, size_y]
#
...
def distribute_rgb(depth, _x, _y, size_x, size_y, image_height, image_width):
    ...
    # [batch, num_points, size_x, size_y]
    color_value = value.view([batch, -1])
    ...
    true_coordinates = (true_extended_x.view([batch, -1]) * image_width) + true_extended_y.view(
        [batch, -1])
    true_coordinates[~masked_points.view([batch, -1])] = coord_max
    colorvalue_scattered = torch.zeros(
        [batch, image_width * image_height],
        device=depth.device).scatter_add(1, coordinates.long(), color_value)
    return colorvalue_scattered


def points2depth2(points, image_height, image_width, size_x=4, size_y=4):
    """
    ...
    epsilon = torch.tensor([1e-3], requires_grad=False, device=points.device)
    min_vals = torch.min(points, dim=-2, keepdim=True)[0] - epsilon
    max_vals = torch.max(points, dim=-2, keepdim=True)[0] + epsilon
    ...
    depth_recovered[:, _x[0], _y[0]] = points[:, :, 2].float()
...
def points2image(points, image_height, image_width, size_x=4, size_y=4):
    ...
    for i in range(points.shape[-1] - 3):
        depth = points[:, :, 3 + i]
        color_scattered = distribute_rgb(
            depth=depth,
            _x=_x,
            _y=_y,
            size_x=size_x,
            size_y=size_y,
            image_height=image_height,
            image_width=image_width).view([batch, image_height, image_width])  # [B, image_width, image_height]
        scatter_RBG.append(color_scattered)
    return torch.stack(scatter_RBG, dim=-1).permute(0, 3, 1, 2)  # [600, 128, 128, 3]->[600, 3, 128, 128]
    # return depth_recovered
...
class PCViews:
    ...
    def get_img(self, points, resolution=None):
        ...
        Args:
            points (torch.tensor): of size [B, _, 3]   (1,800,3)
        Returns:
        ...
        v = int(self.translation.shape[0])

        _points = self.point_transform(
            points=torch.repeat_interleave(points, v, dim=0),
            rot_mat=self.rot_mat.repeat(b, 1, 1),
            translation=self.translation.repeat(b, 1, 1))  # [600, 1024, 3]
...
class RGBPCViews:
    ...
    def get_img(self, xyz, rgb):
        """Get image based on the prespecified specifications.

        Args:
            points (torch.tensor): of size [B, _, 6]  (xyzrgb)  rgb normed
        Returns:
            img (torch.tensor): of size [B * self.num_views, RESOLUTION,
                RESOLUTION]
        """
        ...
        _points = torch.repeat_interleave(points, v, dim=0)
        _points[:, :, :3] = self.point_transform(
            points=torch.repeat_interleave(points[:, :, :3], v, dim=0),
            rot_mat=self.rot_mat.repeat(b, 1, 1),
            translation=self.translation.repeat(b, 1, 1))  # [600, 1024, 3]

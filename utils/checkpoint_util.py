...
def load_pretrain_checkpoint(model, pretrain_checkpoint_path):
    ...
    if pretrain_checkpoint_path is not None:
        ...
        aaa=model.load_state_dict(model_dict)
        if pretrained_dict.keys().__len__()>0 and aaa:
            print('\t-----------successful load pretrained', pretrained_dict.keys().__len__())
        else:
            print('\n-----------failed load pretrained')
    else:

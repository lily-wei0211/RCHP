...
def metric_evaluate(predicted_label, gt_label, NUM_CLASS,logger):
    """
    ...
    logger.cprint('Overall accuracy: {0}'.format(oa))
    iou_list = []
    ...
    for i in range(NUM_CLASS):
        denominator = float(gt_classes[i] + positive_classes[i] - true_positive_classes[i])
        if denominator == 0:  # if devide by 0
            iou_class = 0.0  # set IOU to 0
        else:
            iou_class = true_positive_classes[i] / denominator

        # print('Class_%d: iou_class is %f' % (i, iou_class))  # 打印调试信息
        logger.cprint('Class_%d: iou_class is %f' % (i, iou_class))  # 打印调试信息
        iou_list.append(iou_class)
...
def pretrain(args):
    ...
    if args.dataset == 's3dis':
        ...
        from dataloaders.scenenn import SceneNNDataset
        DATASET = SceneNNDataset(args.cvfold, args.data_path)
    elif args.dataset == 'nyudepthv2':
        from dataloaders.nyudepthv2 import NYUDepthV2Dataset
        DATASET = NYUDepthV2Dataset(args.cvfold, args.data_path)
    elif args.dataset == 'semantic3d':
        from dataloaders.semantic3d import Semantic3DDataset
        DATASET = Semantic3DDataset(args.cvfold, args.data_path)
    else:
    ...
    if args.pretrain_name == 'DGCNN':
        model = DGCNNSeg(args, num_classes=NUM_CLASSES)
    elif args.pretrain_name=='stratified':
        from models_transformer.stratified_transformer import Stratified
        model = Stratified(args, num_classes=NUM_CLASSES)
    elif args.pretrain_name == 'pct_partseg':
        # from models_transformer.point_transformer_seg import PointTransformerSeg26,PointTransformerSeg38,PointTransformerSeg50
        from models_transformer.pct_partseg import Point_Transformer_partseg
        model = Point_Transformer_partseg(in_dim=args.pc_in_dim, num_classes=NUM_CLASSES)
    ...
    for epoch in range(args.n_iters):
        ...
        if (epoch+1) % args.eval_interval == 0 or (epoch+1)==args.n_iters:
            pred_total = []
            gt_total = []
            model.eval()
            with torch.no_grad():
            ...
            accuracy, mIoU, iou_perclass = metric_evaluate(pred_total, gt_total, NUM_CLASSES,logger)
